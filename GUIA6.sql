...
SELECT
    ...
    TO_CHAR(SUM(MONTO_CREDITO), '$99G999G999') AS "MONTO TOTAL CREDITOS"
...
SELECT
    TO_CHAR(CLI.NUMRUN, '99G999G999')||'-'|| CLI.DVRUN AS "RUN CLIENTE",
    CLI.PNOMBRE ||' '|| CLI.SNOMBRE ||' '|| CLI.APPATERNO ||' '|| CLI.APMATERNO AS "NOMBRE CLIENTE",
    (CASE WHEN MIN(MOV.COD_TIPO_MOV) = 1 THEN TO_CHAR(SUM(MONTO_MOVIMIENTO), '$99G999G999') ELSE 'NO REALIZO' END) AS "ABONO",
    (CASE WHEN MAX(MOV.COD_TIPO_MOV) = 2 THEN TO_CHAR(SUM(MONTO_MOVIMIENTO), '$99G999G999') ELSE 'NO REALIZO' END) AS "RETIRO"    
FROM CLIENTE CLI JOIN MOVIMIENTO MOV ON (CLI.NRO_CLIENTE = MOV.NRO_CLIENTE)
